...
namespace ExRam.Gremlinq.Samples.Shared {
    public class Logic {
        ...
        private async Task Create_the_graph() {
            ...
            _marko = await _g.AddV(new Person {Name = "Marko", Age = 29})
                .FirstAsync();

            _vadas = await _g.AddV(new Person {Name = "Vadas", Age = 27})
                .FirstAsync();

            _josh = await _g.AddV(new Person {Name = "Josh", Age = 32})
                .FirstAsync();

            _peter = await _g.AddV(new Person {Name = "Peter", Age = 35})
                .FirstAsync();
            ...
            var charlie = await _g.AddV(new Dog {Name = "Charlie", Age = 2})
                .FirstAsync();

            var catmanJohn = await _g.AddV(new Cat {Name = "Catman John", Age = 5})
                .FirstAsync();

            var luna = await _g.AddV(new Cat {Name = "Luna", Age = 9})
                .FirstAsync();

            var lop = await _g.AddV(new Software {Name = "Lop", Language = ProgrammingLanguage.Java})
                .FirstAsync();

            var ripple = await _g.AddV(new Software {Name = "Ripple", Language = ProgrammingLanguage.Java})
                .FirstAsync();

            await _g.V(_marko.Id!)
                .AddE<Knows>()
                .To(__ => __.V(_vadas.Id!))
                .FirstAsync();
            ...
            await _g.V(_daniel.Id!)
                .AddE<Owns>()
                .To(__ => __.V(catmanJohn.Id!))
                .FirstAsync();
        ...
        private async Task Create_vertices_and_a_relation_in_one_query() {
            ...
            await _g.AddV(new Person {Name = "Bob", Age = 36})
                .AddE<Knows>()
                .To(__ => __.AddV(new Person {Name = "Jeff", Age = 27}))
                .FirstAsync();
        }
        ...
        private async Task Who_does_Marko_know() {
            ...
            var knownPersonsToMarko = await _g.V(_marko!.Id!)
                .Out<Knows>()
                .OfType<Person>()
        ...
        private async Task Who_is_older_than_30() {
            ...
            var personsOlderThan30 = await _g.V<Person>()
                .Where(x => x.Age > 30);
        ...
        private async Task Whose_name_starts_with_B() {
            ...
            var nameStartsWithB = await _g.V<Person>()
        ...
        private async Task Who_knows_who() {
            ...
            var friendTuples = await _g.V<Person>()
                .As((__, person) => __.Out<Knows>()
                    .OfType<Person>()
                    .As((__, friend) => __.Select(person, friend)));
        ...
        private async Task Who_does_what() {
            ...
            var tuples = await _g.V<Person>()
                .As((__, person) => __.OutE<Edge>()
                    .As((__, edge) => __.InV<Vertex>()
                        .As((__, what) => __.Select(person, edge, what))));
        ...
        private async Task How_many_pets_does_everybody_have() {
            ...
            var dynamics = await _g.V<Person>()
                .Project(b => b.ToDynamic()
        ...
        private async Task Who_has_that_phone_number() {
            ...
            var personWithThatPhoneNumber = await _g.V<Person>()
                .Where(person => person.PhoneNumbers!.Contains("+491234567"))
                .FirstOrDefaultAsync();
        ...
        private async Task Who_has_a_phone() {
            ...
            var personsWithPhoneNumber = await _g.V<Person>()
                .Where(person => person.PhoneNumbers!.Any());
        ...
        private async Task What_entities_are_there() {
            ...
            var entityGroups = await _g.V()
                .Group(g => g.ByKey(__ => __.Label())
                    .ByValue(__ => __.Count()))
                .FirstAsync();
        ...
        private async Task Who_created_some_software() {
            ...
            var creators = await _g.V<Person>()
                .OutE<Created>()
                .OutV()
                .Dedup();
        ...
        private async Task Whose_age_is_29_30_or_31() {
            ...
            var personsWithSpecificAges = await _g.Inject(29, 30, 31)
                .Fold()
                .As((_, ages) => _.V<Person>()
